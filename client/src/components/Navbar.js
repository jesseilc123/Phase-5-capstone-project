...
function Navbar() {
    const { view, setView, user, setUser } = useContext(UserContext);
        ...
        <div className={`fixed top-0 flex-row z-[100] h-24 w-full bg-grey ${location.pathname==="/login" ? "hidden" : "flex"}`}>
            ...
            <div className="flex w-full items-center justify-end">
                {user ? (
                    ...
                    <button className="text-white text-xl p-3 hover:text-orange-300" onClick={handleLogout}>Logout</button>
                    </>
                ) : (
                    <>                
